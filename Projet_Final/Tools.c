...
void addHighscore(char *username, int time) {
    ...
    struct HighscoreEntry scores[MAX_TIMES + 1];
    int count = 0;

    // Lire les scores existants
    while (fscanf(file, "%s %d\n", scores[count].name, &scores[count].time) == 2 && count < MAX_TIMES) {
        count++;
    }

    // Ajouter le nouveau score s'il est meilleur que les précédents
    int index = -1;
    ...
    if (index != -1) {
        // Décaler les scores existants vers le bas pour faire de la place pour le nouveau score
        ...
        strcpy(scores[index].name, username);
        scores[index].time = time;

        // Écrire les scores triés dans le fichier
        fseek(file, 0, SEEK_SET);
        for (int i = 0; i < MAX_TIMES; i++) {
            fprintf(file, "%s %d\n", scores[i].name, scores[i].time);
        }
    }
